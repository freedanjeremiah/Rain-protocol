...
fun test_borrow_order_getters() {
    ...
    let borrower = @0x1234;
    let order = create_borrow_order_for_testing(borrower, 1000, 500, 86400, &mut ctx);
...
fun test_partial_fill_mutators() {
    ...
    let borrower = @0x1234;
    let lender = @0x5678;
    let mut borrow_order = create_borrow_order_for_testing(borrower, 1000, 500, 86400, &mut ctx);
